...
1. **`holdout-24/` — frozen final holdout (it-87, однократное открытие).** Спека полная:
   ...
   ```
   Длительность КАЖДОГО сегмента — строго ≥60 с: pre-flight раннера отказывает на более
   коротком (шов 21: на dur < ~50 с ratio покрытия H0.4 завышается математически — ложный
   H0-блок однократного открытия). Видео — строго `.mp4`, аудио — строго `.wav` (шов 22:
   то же расширение, что читает simulator; раннер ОТКАЗывает в pre-flight ДО замера).
   Суммарно: ≥5 позитивов и ≥600 с негативов (сумма dur ролей neg) — тоже механические
   гейты. Доп. причина ≥60 с: mono-плечо V0 замера it-86-full (шаг 5 после it-87) на
   сегментах <40 с деградирует (watermark-релиз только событийным max_wait — находка 6
   отчёта it-86).
