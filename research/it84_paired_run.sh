#!/usr/bin/env bash
# it-84 — exact paired replay: A off -> B AND@0,4 на ОДНИХ 400 HD-фонах (loop=false),
# конец этапа по дренажу (поток решений стих 45 с), не по wall-таймеру (урок it-83:
# хвостовые решения срезаются таймером). Гейты it-81/82/83 сохранены.
# Heredoc-правило: аргументы в write_override — без экранирования; оверлей печатается.
set -u
ROOT=/home/otrix/code/GeneralFolderMasterDiploma
MD=$ROOT/MasterDiploma
cd "$MD"

OVR="$MD/infra/docker-compose.it84.yml"
JSONL="$MD/data/decisions/decisions.jsonl"
LOG="$ROOT/research/it84_paired_run.log"
...
stage() {  # $1=имя, $2=vote_mode, $3=floor, $4=ожидаемый model_name (regex)
  ...
  write_override "$2" "$3"
  echo "--- оверлей (самопроверка heredoc):"; grep -E "VOTE_MODE|LOOP" "$OVR"
  ...
  echo "-- P4-гейт OK: $P4M"
  # дренажное завершение: 400 кадров @2fps = ~200 с + тёп-запуск; ждём тишины 45 с, максимум 340 с
  PREV=$(offset); STABLE=0; DEADLINE=$((SECONDS+340))
  while [ $SECONDS -lt $DEADLINE ]; do
    sleep 15
    CUR=$(offset)
    if [ "$CUR" -eq "$PREV" ]; then STABLE=$((STABLE+15)); else STABLE=0; fi
    PREV=$CUR
    if [ "$STABLE" -ge 45 ]; then break; fi
  done
  NEW=$((CUR-OFF_BEFORE))
  [ "$NEW" -ge 200 ] || { echo "ОТКАЗ дренажа: '$1' дал $NEW решений (<200)"; exit 1; }
  OFF_AFTER=$CUR
  echo "SCORE_OFF $1 $((OFF_BEFORE+1)):$OFF_AFTER (новых $NEW, тишина ${STABLE}с)"
  "${COMPOSE[@]}" rm -sf $SERVICES >/dev/null 2>&1 || true
}

exec > >(tee -a "$LOG") 2>&1
echo "=== it-84 paired replay $(date '+%F %T') ==="
[ -d "$MD/train/data/_prepared/hi-res-bg-v1/strict400" ] || { echo "ОТКАЗ: нет strict400"; exit 1; }
echo "[it84] sha весов: $(sha256sum models/visual/yolov8s-uav.pt models/visual/uav-yolov8s-bg-best.pt | cut -c1-12 | tr '\n' ' ')"
...
echo "[it84] ждём инфраструктуру (30с)..."; sleep 30
create_topics() {
  local t have
...
rm -f "$OVR"
echo
echo "=== it-84 завершён; разбор: research/.venv/bin/python research/it84_paired_eval.py <срезы SCORE_OFF> ==="
