...
research/.venv/bin/python research/verify_manifest.py  # сверить sha256 всех файлов манифеста + gitignored-входы (сейчас 154+3; exit 0 = целостно)
```
...
Семейство иных постоянных warning'ов зарегистрировано заранее и тоже безвредно (сверено 24.09 при
pre-flight it-87): `*-bg70.*`/`train/runs/…/uav-yolov8s-bg70/…` — ключи планового плеча T it-70,
закрытого без исполнения решением автора (план 22.09 п.4); `mmaud_sahi_full_{doguilmak-v11x,
ruju-v12, danivelikova-v26n, iris-v8s}.csv` — резерв арбитража S-плеча it-70, MMAUD-арбитраж выполнен
только для топ-3 (noah-v8s, doguilmak-v8x, skyguard-v11; ruju-v12 — мёртвый вес). Ключи оставлены
как след предрегистрации; при regen stderr-список «не включены» — штатный, не потеря артефактов.
...
# ПЕРЕЗАТВЕРЖДЕНИЕ 24.09 (вечер): тот же прогон со стандартной verbosity → фактическая
# итоговая строка «99 passed in 11.67s» (прежняя фиксация была по сумме точек/коллекции).
# ПЕРЕПРОВЕРКА 24.09 (ночь, пре-открыточный hardening): та же каноническая команда →
# «99 passed in 9.32s» (тестовые файлы с вечерней фиксации bc52765 не менялись — git diff пуст).
# Отдельный факт (не канон §6n): голый `venv/bin/python -m pytest` из корня MasterDiploma
# собирает шире контура §6n (захватывает train/tests) → «103 passed in 9.84s»; сравнивать
# 103 с 99 некорректно — контур §6n остаётся 99.
```
...
#   Шов 20 (полный смоук 24.09): глобальная media_ts-монотонность H0.3 невыполнима конструктивно
#   (merge-адаптер: watermark-выпуск аудио идёт между видео-окнами с опережающим media_ts) →
#   H0.3 = монотонность по порядку потока ВНУТРИ подпотоков аддитивного поля trigger_modality
#   (messages.py, fusion _emit; аттестация P6b в раннере; пороги не тронуты; selftest +сценарий D).
#   Шов 21 (тот же смоук): ratio H0.4 завышается на dur<~50 с (30-с pos → 1,376 = ложный
#   H0-блок) → pre-flight гейт MIN_DUR=60 с на каждый dur + боевой страж (допуск записи,
#   eval-семантика не тронута; смоук-материал pos перегенерирован на 65 с).
#   Живая валидация шва 20 (перезапуск смоука smoke18, 24.09): P6b пройден, все строки
#   размечены (video/audio, null=0), mono(по подпотокам)=OK на всех 7 сегментах; осечка H0
#   только на ratio 30-с pos (шов 21, закрыт: гейт+страж живо проверены, pos-материал → 65 с).
#   Шов 22: шаг 1в новизны — глоб *.{mp4,wav} проскакивал .mov/.m4a → чек-лист на find по
#   каталогу кроме манифеста + механический pre-flight гейт .mp4/.wav в раннере (зонд-аттестация
#   в обе стороны без docker-побочных эффектов).
#   Финальная пре-открыточная репетиция (24.09, smoke18 pos 65 с ×5 + neg 300 с ×2, rc=0):
#   ВПЕРВЫЕ H0 (блокаторы) = ПРОЙДЕН на всём харнессе — ratio pos 1,137 / neg 1,029,
#   mono 7/7, p_a≠null 100 %, sha-канон цел; H1–H3 зелёные, но тепличные (вердикт — только
#   в бою). Боевые пути (holdout-лог/summary/csv) чисты; единственный блокер — материал автора.
```

## 7. Скан-ритуал коммита (обязателен для КАЖДОГО коммита, не только «важных»)

Дефект-класс: в русскоязычный текст проскакивает CJK/омоглиф или смешение скриптов
(задокументированный случай — U+8BEF в сообщении 272521c; amend запрещён дисциплиной,
дефект неизгладим в публичной истории). Ритуал встраивается в ТУ ЖЕ командную цепочку,
что и коммит — отдельным шагом он не исполняется (урок LESSONS seen ×2):

```bash
MSG='…сообщение…'
git add <файлы>
if grep -nP '^[+].*([A-Za-z]\p{Cyrillic}{2,}|\p{Cyrillic}[A-Za-z]{2,}|[\p{Han}\p{Hiragana}\p{Katakana}\p{Hangul}])' <(git diff --cached) \
   || printf '%s' "$MSG" | grep -qP '[\p{Han}\p{Hiragana}\p{Katakana}\p{Hangul}]|[A-Za-z]\p{Cyrillic}|\p{Cyrillic}[A-Za-z]'; then
  echo "СКАН: находка — коммит отменён"; git reset --quiet; exit 1
fi
git commit -m "$MSG"
```

Ветвь MSG без `{2,}` — намеренно строже diff-ветви: одиночная латинская буква при
кириллице в сообщении тоже подозрение (находка разбирается вручную, а не молча
ослабляется). Тот же класс воспроизвёлся в пользовательских сообщениях сессии
(иероглиф и польская буква в чате) — git-скан от него не защищает, текст сообщения
проверяется тем же паттерном до отправки. Смешение через дефис (`out-дрейф`) —
допустимая норма, паттерн её не ловит.
