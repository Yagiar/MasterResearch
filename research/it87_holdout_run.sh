...
# drift стека между заморозкой и приходом материала убивал бы однократное открытие уже ПОСЛЕ
# замера — поднимаем проверку в pre-flight, до tee/docker (урок класса «проверка была устной/
# постфактум — делаем механической и до побочных эффектов»; шаг 1а чек-листа остаётся сверкой,
# но теперь защищён и от забывчивости оператора).
for wf in "yolov8s-uav.pt 41f3fd55" "uav-yolov8s-bg-best.pt 7042602a"; do
  set -- $wf
  [ -f "$MD/models/visual/$1" ] || { echo "ОТКАЗ pre-flight: нет веса $MD/models/visual/$1 — боевой стек не соберётся, H0.1 eval мёртв после замера"; exit 1; }
  got=$(sha256sum "$MD/models/visual/$1" | cut -c1-8)
  [ "$got" = "$2" ] || { echo "ОТКАЗ pre-flight: sha256 $1 = ${got}…, канон ${2}… — замороженный стек дрейфовал, однократное открытие не состоится (шаг 1а чек-листа)"; exit 1; }
done
...
while IFS=$'\t' read -r id role vid aud dur gs ge; do
  ...
  [[ "$id" == -* ]] && { echo "ОТКАЗ pre-flight: segment_id '$id' — ведущий дефис: срез '$id=…' из SCORE_OFF попал бы в eval-парсер как '--…'-флаг (id с '--' молча съел бы свой срез — полнота упала бы в H0-ПРОВАЛ уже ПОСЛЕ однократного замера)"; exit 1; }
  [[ -n "${SEENID[$id]:-}" ]] && { echo "ОТКАЗ pre-flight: segment_id '$id' повторяется — eval собирает сегменты в словарь по id, дубликат молча съедал бы пролёт/gt у H2 после необратимого замера"; exit 1; }
  SEENID[$id]=1
