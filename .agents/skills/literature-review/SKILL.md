...
> **Local adaptation (NIR-2 UAV thesis workspace):** the `paper-search` skill and the author's hardcoded
> `/Users/shuai/...` path are not available here. Go directly to WebSearch/WebFetch, the
> Semantic Scholar / Crossref / OpenAlex / arXiv HTTP APIs, or an installed paper-search MCP server.
